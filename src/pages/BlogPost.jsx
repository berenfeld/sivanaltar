...
export default function BlogPost() {
  ...
  const [form, setForm] = useState({ title: "", summary: "", content: "", category: "הגיגים", published: false, image_url: "" });
  ...
  const loadPost = async () => {
    ...
    if (items.length > 0) {
      const p = items[0];
      setPost(p);
      setForm({ title: p.title || "", summary: p.summary || "", content: p.content || "", category: p.category || "הגיגים", published: p.published || false, image_url: p.image_url || "" });
    }
    setLoading(false);
  };
    ...
    return (
      <div dir="rtl" className="min-h-screen bg-[#f8f5f0]">
        ...
        <div className="max-w-3xl mx-auto px-6 py-10">
          ...
          {post.category && (
            <span className="text-xs bg-[#f0e8d8] text-[#c8a96e] px-2 py-0.5 rounded-full font-medium mr-2">{post.category}</span>
          )}
          <h1 className="text-3xl font-bold text-[#3a3a4a] mt-3 mb-2 leading-snug">{post.title}</h1>
          {post.publish_date && (
            <p className="text-sm text-[#aaa] mb-6">
              עודכן: {new Date(post.publish_date).toLocaleDateString("he-IL", { year: "numeric", month: "long", day: "numeric" })}
            </p>
          )}
          <div
            className="prose prose-lg max-w-none text-[#3a3a4a] leading-relaxed [&>ul]:list-disc [&>ul]:pr-6"
            dir="rtl"
  ...
  if (!isAdmin) {
    return (
      <div dir="rtl" className="flex items-center justify-center min-h-screen">
        <p className="text-[#999]">אין גישה לעמוד זה</p>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {form.published ? <><Eye size={14} /> מפורסם</> : <><EyeOff size={14} /> לא מפורסם</>}
            </button>
            <button
              onClick={handleSave}
              disabled={saving}
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 space-y-5">
          ...
          <div>
            {form.image_url ? (
              <div className="relative">
                <img src={form.image_url} alt="תמונה" className="w-full h-48 object-cover rounded-xl" />
                <button
              ...
              <label className="flex flex-col items-center justify-center h-36 border-2 border-dashed border-[#c8a96e] rounded-xl cursor-pointer hover:bg-[#fdf8f0] transition-colors">
                ...
                <span className="text-sm text-[#999]">{uploadingImage ? "מעלה..." : "העלה תמונה לכותרת"}</span>
          ...
          <input
            ...
            placeholder="קטגוריה (למשל: הגיגים)"
          ...
          <textarea
            ...
            placeholder="תקציר קצר (מוצג ברשימת הבלוג)"
            rows={2}
