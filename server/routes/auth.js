...
router.get('/me', async (req, res) => {
  ...
  try {
    ...
    // Fetch live lang from DB (not in JWT to avoid stale values)
    const result = await pool.query('SELECT lang FROM users WHERE id = $1', [user.userId]);
    const lang = result.rows[0]?.lang || 'he';
    res.json({
      id: user.userId,
      email: user.email,
      full_name: user.name,
